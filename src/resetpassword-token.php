...
<header>
    ...
    <form method="post" action="resetpassword-token.php">
        ...
        <div class="input-group">
            ...
            <label>
                <input type="password" required="required" id="newPassword" name="newPassword" oninput="checkPasswordStrength(document.getElementById('newPassword').value, '<?php echo htmlspecialchars(getEmailFromToken(hex2bin($_REQUEST['token'] ?? ''))) ?>')">
            </label>
        </div>
        <div class="input-group">
            <label>Rewrite password</label>
            <label>
                <input type="password" required="required" id="newPasswordRetype" name="newPasswordRetype" oninput="checkPasswordStrength(document.getElementById('newPassword').value, '<?php echo htmlspecialchars(getEmailFromToken(hex2bin($_REQUEST['token'] ?? ''))) ?>')">
            </label>
        </div>
